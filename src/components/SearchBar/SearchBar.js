...
export default function SearchBar({ data }) {
  ...
  let countries = Array.from(
    new Set(data.map((tempObject) => tempObject.country))
  );
  ...
  return (
    <Container fluid className="body">
      ...
      <Row noGutters className="filters">
        ...
        <Dropdown className="dropdowns">
          ...
          <Dropdown.Menu>
            {genres.map((genre, index) => (
              <Dropdown.Item onClick={() => setGenre(genre)} key={index}>
                {genre}
              </Dropdown.Item>
            ))}
        ...
        {genre === "" ? (
          <h2 className="selectedText"> None </h2>
        ) : (
          <h2 className="selectedText"> {genre} </h2>
        )}
        ...
        <Dropdown className="dropdowns">
          ...
          <Dropdown.Menu>
            {countries.map((country, index) => (
              <Dropdown.Item onClick={() => setCountry(country)} key={index}>
                {country}
              </Dropdown.Item>
            ))}
        ...
        {country === "" ? (
          ...
          <h2 className="selectedText"> {country} </h2>
        )}
